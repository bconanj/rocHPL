  ...
  if(block == 0) {
    ...
    for(int jj = 0; jj < JB; ++jj) {
      ...
      if(t < NB) {
        ...
        atomic_ref<double, thread_scope_device> L1tj(L1[t * NB + jj]);
    ...
    for(int jj = 1; jj < JB; ++jj) {
      ...
      if(t < NB) {
        ...
        atomic_ref<double, thread_scope_device> L1tj(L1[t * NB + (jj - 1)]);
...
void HPL_pdpanrlN(HPL_T_panel* PANEL,
  ...
  if(M > 0) {
    ...
    void* params[] = {&m,
                      ...
                      &host_flag,
                      &barrier};
    CHECK_HIP_ERROR(hipLaunchCooperativeKernel(pdpanrlN<BLOCKSIZE>,
                                               dim3(Nblocks),
                                               dim3(BLOCKSIZE),
                                               params,
                                               0,
